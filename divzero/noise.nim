...
func valueRand*(x: float32): float32 =
  ## [0..1]
  fract(sin(x) * 43758.5453123f)

func valueRand*(x, y: float32): float32 =
  ## [0..1]
  let d = x * 12.9898f + y * 78.233f
  fract(sin(d) * 43758.5453123f)

func valueNoiseLinear*(x: float32): float32 =
  ## [0..1]
  let i = floor(x)
  let f = fract(x)
  lerp(f, value_rand(i), value_rand(i + 1f))

func valueNoise*(x: float32): float32 =
  ## [0..1]
  let i = floor(x)
  let f = fract(x)
...
func fbmNoise*(x, y: float32; octaves: int): float32 =
  var a = 0.5f
  var m = 0f
  var i = 0
  var x = x
