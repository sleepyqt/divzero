...
func quatRX*(angle: float32): Quat =
  result.x = sin(angle * 0.5f)
  result.y = 0f
  result.z = 0f
  result.w = cos(angle * 0.5f)

func quatRY*(angle: float32): Quat =
  result.x = 0f
  result.y = sin(angle * 0.5f)
  result.z = 0f
  result.w = cos(angle * 0.5f)

func quatRZ*(angle: float32): Quat =
  result.x = 0f
  result.y = 0f
  result.z = sin(angle * 0.5f)
  result.w = cos(angle * 0.5f)

func lenSq*(quat: Quat): float32 =
...
func toMatrix*(rot: Quat; pos: Vec4): Mat4 =
  result = to_matrix(rot)
  result.c3 = pos

func toAxisAndAngle*(q: Quat): (Vec4, float32) =
  let ca = q.w
...
func rotateVector*(q: Quat; v: Vec4): Vec4 =
...
func rotatePoint*(q: Quat; v: Vec4): Vec4 =
  let vt = q * quat(v.x, v.y, v.z, 0f) * conjugate(q)
  result.x = vt.x
  result.y = vt.y
  result.z = vt.z
...
func sLerp*(t: float32; a, b: Quat): Quat =
  ...
  if (1f - abs_cosom) > 1e-6f:
    ...
    scale0 = sinPoly5((1f - t) * omega) * sinom
    scale1 = sinPoly5(t * omega) * sinom
  else:
    scale0 = 1f - t
    scale1 = t
...
func dualQuat*(): DualQuat =
  result.real = quat(0f, 0f, 0f, 1f)
  result.dual = quat(0f, 0f, 0f, 0f)

func dualQuat*(real, dual: Quat): DualQuat =
  result.real = normalize(real)
  result.dual = dual

func dualQuat*(rotation: Quat; t: Vec4): DualQuat =
  result.real = normalize(rotation)
  result.dual = quat(t.x, t.y, t.z, 0f) * result.real * 0.5f

func dualQuatT*(t: Vec4): DualQuat =
  result.real = quat()
  result.dual = quat(t.x, t.y, t.z, 0f) * result.real * 0.5f

func dualQuatT*(x, y, z: float32): DualQuat =
  result.real = quat()
  result.dual = quat(x, y, z, 0f) * result.real * 0.5f

func dualQuatRX*(angle: float32): DualQuat =
  result.real = quat_rx(angle)
  result.dual = quat(0f, 0f, 0f, 0f)

func dualQuatRY*(angle: float32): DualQuat =
  result.real = quat_ry(angle)
  result.dual = quat(0f, 0f, 0f, 0f)

func dualQuatRZ*(angle: float32): DualQuat =
...
func rotateVector*(q: DualQuat; v: Vec4): Vec4 =
...
when isMainModule:
  block:
    ...
    let q1 = quat(2, 3, 4, 1)
    let q2 = quat(6, 7, 8, 5)
    doAssert(q1 * q2 == quat(12.0, 30.0, 24.0, -60.0))
  ...
  block:
    ...
    let q3 = dualQuat(quat(), vec4(1, 2, 3, 1))
    echo "q3"
    echo q3.to_matrix.pretty
    let q4 = dualQuat(quat(vec4(0, 1, 0, 0), 1.23f), vec4())
    echo "q4"
    echo q4.to_matrix.pretty
    let q5 = dualQuat(quat(vec4(0, 1, 0, 0), 1.23f), vec4(1, 2, 3, 1))
    echo "q5"
    echo q5.to_matrix.pretty
    let q6 = dualQuat(quat(), vec4(3, 2, 1, 1))
    echo "q6"
