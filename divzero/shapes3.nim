...
func plane3CW*(a, b, c: Vec4): Plane3 =
...
func plane3CCW*(a, b, c: Vec4): Plane3 =
...
proc rayCast*(ray: Ray3; plane: Plane3; info: var RaycastInfo) =
...
proc rayCast*(ray: Ray3; sphere: Sphere; info: var RaycastInfo) =
...
proc rayTest*(ray: Ray3; box: AABB3): bool =
...
func screen_ray*(x, y, width, height: float32; invPv: Mat4; length: float32): Ray3 =
  ## `x`, `y` - point on screen
  ## `width`, `height` - screen size
  ## `length` - ray length
  ## `invPv` - clip-to-world matrix
  let clipN = toCartesianPoint(invPv * windowToNearClip(x, y, width, height))
  let clipF = toCartesianPoint(invPv * windowToFarClip(x, y, width, height))
  ray3(clipN, direction(clipN, clipF) * length)

# Frustum

func fromProjViewMatrix*(m: Mat4): Frustum =
